...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedWalkingInMsec := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedWalkingInMsec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
